...
from tools import get_weather, search_places, search_hotels, track_flight
...
4. **Flights** ✈️: Track live flight status by flight number (e.g. AA123)
...
- For flight tracking, provide the status, departure, and arrival times clearly
...
- For flight tracking, you must have the airline flight number (e.g., AA123). If the user doesn't provide it, ask for it.
...
def create_agent() -> AgentExecutor | None:
    ...
    """
    if not Config.get_api_status()["Gemini"]:
        return None
